...
def transformer(U, theta, out_size, name='SpatialTransformer', **kwargs):
    ...
    def _interpolate(im, x, y, out_size,scale_h):
        ...
        # constants
        num_batch, height, width, channels = im.shape#tf.shape(im)

        x = tf.cast(x, 'float32')
        y = tf.cast(y, 'float32')
        
        height_f = tf.cast(x=height, dtype='float32')
        width_f = tf.cast(x=width, dtype='float32')
        
        out_height, out_width = out_size[0:2]
        ...
        x0 = tf.clip_by_value(x0, clip_value_min=zero, clip_value_max=max_x)
        x1 = tf.clip_by_value(x1, clip_value_min=zero, clip_value_max=max_x)
        y0 = tf.clip_by_value(y0, clip_value_min=zero, clip_value_max=max_y)
        y1 = tf.clip_by_value(y1, clip_value_min=zero, clip_value_max=max_y)
        dim2 = width
        dim1 = width*height
        ...
        output = tf.math.add_n([wa*Ia, wb*Ib, wc*Ic, wd*Id])
        return output
    ...
    def _transform(theta, input_dim, out_size, scale_h):
        # print("#####################")
        # print(input_dim.shape, theta.shape)
        # print("#####################")
        num_batch, height, width, num_channels = input_dim.shape#tf.shape(input_dim)
        ...
        smallers = 1e-6*(one - tf.cast(tf.math.greater_equal(tf.abs(t_s_flat), small), tf.float32)) 
            
        
        t_s_flat = t_s_flat + smallers 
        condition =  tf.math.reduce_sum(tf.cast(tf.math.greater(tf.abs(t_s_flat), small), tf.float32))
        # Ty changed 
